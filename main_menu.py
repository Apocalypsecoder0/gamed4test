...
class MainMenuGUI:
    def __init__(self):
        ...
        self.game_world = None  # Ensure game_world attribute always exists
        self.game_stats = {'characters_created': 0}  # Initialize game_stats dictionary
    ...
    def create_menu(self):
        # Diablo 4-inspired dark theme and stylized menu
        ...
        btn_bg = "#23201a"
        btn_fg = "#e0c080"
        btn_active_bg = "#40351a"
        btn_active_fg = "#fffbe6"
        btn_font = ("Georgia", 14, "bold")
        borderwidth = 0
        highlightthickness = 0
        # Title
        tk.Label(frame, text="ARCANE ENGINE: ETERNAL QUEST", font=title_font, fg=label_fg, bg="#181818").pack(pady=(0,2))
        tk.Label(frame, text="by Shadow Studios", font=subtitle_font, fg="#b0a080", bg="#181818").pack()
        tk.Label(frame, text="Engine: Arcane Engine v1.0", font=("Georgia", 10, "italic"), fg="#b0a080", bg="#181818").pack(pady=(0,18))

        def style_button(btn):
            btn.configure(
                bg=btn_bg, fg=btn_fg, font=btn_font, activebackground=btn_active_bg, activeforeground=btn_active_fg,
                relief=tk.FLAT, borderwidth=borderwidth, highlightthickness=highlightthickness, cursor="hand2"
            )
            def on_enter(e):
                btn["bg"] = btn_active_bg
                btn["fg"] = btn_active_fg
            def on_leave(e):
                btn["bg"] = btn_bg
                btn["fg"] = btn_fg
        ...
        menu_buttons = [
            ...
            ("Patch Notes", self.show_patch_notes),
            ("Character System", lambda: messagebox.showinfo("Character System", "Character system not implemented.")),
            ("Inventory System", self.launch_inventory),
            ("Skills System", self.launch_skills),
            ("Quests System", self.launch_quests),
            ("Map System", self.launch_map),
            ("Social System", self.launch_social),
            ("Settings", self.launch_settings),
            ("Game Credits", self.show_credits),
            ("Exit", self.root.quit),
            ("Help / Tutorial", lambda: show_help_window(self.root)),
            ("Season Mode", self.show_season_window),
            ("Pit of Artificers", self.show_pit_window),
            ("Nightmare Dungeons", self.show_nightmare_window),
            # ("Character Select", self.launch_character_select),
            ("Raids", self.launch_raids),
            ("Trials", self.launch_trials),
            ("Dungeons", self.launch_dungeons),
            ("Party Finder", self.launch_party_finder),
            ("Dungeon Finder", self.launch_dungeon_finder),
            ("Teleport", self.launch_teleport),
        ]
    ...
    def _style_submenu(self, win):
        ...
        for widget in win.winfo_children():
            if isinstance(widget, tk.Label):
                widget.configure(bg="#181818", fg="#e0c080", font=("Georgia", 12, "bold"))
            elif isinstance(widget, tk.Button):
                widget.configure(bg="#23201a", fg="#e0c080", font=("Georgia", 12, "bold"),
                                 activebackground="#40351a", activeforeground="#fffbe6", relief=tk.FLAT, cursor="hand2")
            elif isinstance(widget, tk.Frame):
                widget.configure(bg="#181818")
    ...
    def launch_campaign_gameplay(self):
        ...
        for text, cmd in [
            ("Open Map", self.launch_map),
            ("Open Inventory", self.launch_inventory),
            ("Open Quests", self.launch_quests),
            ("Open Skills", self.launch_skills),
            ("Open Social", self.launch_social),
            ("Open Settings", self.launch_settings),
            ("Combat Menu", self.launch_combat_menu),
            ("Crafting Menu", self.launch_crafting_menu),
            ("Exit to Main Menu", gameplay.destroy),
        ]:
            btn = tk.Button(gameplay, text=text, width=22, height=1, command=cmd)
            btn.pack(pady=2)
            self._style_submenu(gameplay)
    ...
    # --- Patch all Toplevel submenus to use _style_submenu ---
    ...
    def launch_inventory(self):
        try:
            ...
            if spec and spec.loader:
                ...
                InventoryGUI = inv_mod.InventoryGUI
                inv = None
                if self.character and 'inventory' in self.character and isinstance(self.character['inventory'], Inventory):
                ...
                raise ImportError('Inventory module not found')
        except Exception as e:
            messagebox.showinfo("Inventory System", f"Inventory system not available: {e}")

    def launch_map(self):
        try:
            from map_system import MapSystem, MapSystemGUI
            map_system = MapSystem()
            gui = MapSystemGUI(map_system)
            gui.run()
        except Exception as e:
            messagebox.showinfo("Map System", f"Map system not available: {e}")

    def launch_skills(self):
        try:
            from skills_system import SkillsSystem, SkillsSystemGUI
            skills_sys = SkillsSystem()
            if self.character and 'skills' in self.character:
                for skill in self.character['skills']:
                    skills_sys.add_skill(skill, 1)
            gui = SkillsSystemGUI(skills_sys)
            gui.run()
        except Exception as e:
            messagebox.showinfo("Skills System", f"Skills system not available: {e}")

    def launch_quests(self):
        try:
            from quests_system import QuestsSystem, QuestsSystemGUI
            quests_sys = QuestsSystem()
            if self.character and 'quests' in self.character:
                for quest in self.character['quests']:
                    quests_sys.add_quest(quest, "Active")
            gui = QuestsSystemGUI(quests_sys)
            gui.run()
        except Exception as e:
            messagebox.showinfo("Quests System", f"Quests system not available: {e}")

    def launch_social(self):
        try:
            from social_system import SocialSystem, SocialSystemGUI
            social_sys = SocialSystem()
            gui = SocialSystemGUI(social_sys)
            gui.run()
        except Exception as e:
            messagebox.showinfo("Social System", f"Social system not available: {e}")

    def launch_settings(self):
        try:
            from settings_system import SettingsSystem, SettingsSystemGUI
            settings_sys = SettingsSystem()
            gui = SettingsSystemGUI(settings_sys)
            gui.run()
        except Exception as e:
            messagebox.showinfo("Settings", f"Settings system not available: {e}")

    def show_patch_notes(self):
        # Simple patch notes dialog
        patch_notes = (
            "Patch Notes:\n"
            "- Added Dungeon Finder and Party Finder systems.\n"
            "- Improved crafting and combat systems.\n"
            "- Fixed various bugs and improved UI.\n"
    ...
    def show_season_window(self):
        ...
        tk.Label(win, text=f"Current Season: {season['name']} (Season {season['number']})").pack(pady=5)
        tk.Label(win, text=f"Start: {season['start']} | End: {season['end']}").pack(pady=5)
        tk.Label(win, text="Features: " + ", ".join(season.get('features', []))).pack(pady=5)
        tk.Button(win, text="Close", command=win.destroy).pack(pady=10)
    ...
    def show_pit_window(self):
        ...
        def complete_pit():
            ...
            msg = f"Completed Pit of Artificers!\nRewards: Gold {rewards['gold']}, XP {rewards['xp']}, Masterwork Materials {rewards['masterwork_materials']}"
            messagebox.showinfo("Pit Complete", msg)
        tk.Button(win, text="Complete Challenge", command=complete_pit).pack(pady=5)
        tk.Button(win, text="Close", command=win.destroy).pack(pady=10)
